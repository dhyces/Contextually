        ...
        if (pos instanceof BlockHitResult blockHitResult) {
            if (hand == null) {
                if (player.getMainHandItem().getItem() instanceof BlockItem blockItem) {
                    var state = blockItem.getBlock().getStateForPlacement(new BlockPlaceContext(level, player, InteractionHand.MAIN_HAND, player.getMainHandItem(), blockHitResult));
                    if (state != null) {
                ...
                if (player.getOffhandItem().getItem() instanceof BlockItem blockItem) {
                    var state = blockItem.getBlock().getStateForPlacement(new BlockPlaceContext(level, player, InteractionHand.OFF_HAND, player.getMainHandItem(), blockHitResult));
                    if (state != null) {
                        return state.canSurvive(level, blockHitResult.getBlockPos());
                    }
                    return false;
            ...
            if (player.getItemInHand(hand).getItem() instanceof BlockItem blockItem) {
                var state = blockItem.getBlock().getStateForPlacement(new BlockPlaceContext(level, player, hand, player.getItemInHand(hand), blockHitResult));
                if (state != null) {
                    return state.canSurvive(level, blockHitResult.getBlockPos());
                }
            }
